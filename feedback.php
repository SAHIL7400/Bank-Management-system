  ...
  <?php
    ...
    if (isset($_POST['userLogin']))
    {
      $error = "";
        ...
        $stmt = $con->prepare("select * from userAccounts where email='$user' AND password='$pass'");
...
<body style="background:#33FFE8;background-size: 100%">
...
<div class="container">
  ...
  <div class="card-body">
    ...
    <?php
    if (isset($_POST['send']))
    { $stmt = $con->prepare("insert into feedback (message,userId) values (?,$_SESSION[userId])");
      $unsafe_variable = $_POST['msg'];
      $stmt->bind_param("s", $unsafe_variable);
      if ($stmt->execute() and $stmt->close()) {
        echo "<div class='alert alert-success'>Successfully send</div>";
      }else
       "<div class='alert alert-danger'>Not sent Error is:".$con->error."</div>";
    }
