    ...
    <?php 
      ...
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='alert alert-danger'>Deduction have been made for  Rs.$row[debit] from your account at $row[date] in case of $row[beneficiaryAcc]</div>";
            }
            if ($row['action'] == 'transfer') 
            {
              echo "<div class='alert alert-warning'>Transfer have been made for  Rs.$row[debit] from your account at $row[date] in  account no.$row[beneficiaryAcc]</div>";
            }
